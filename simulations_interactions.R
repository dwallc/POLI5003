...
states.st <- scale(states[,c(1,3:9)]) # scale() standardizes variables (subtracts the mean and divides by sd)
...
m2 <- lm(regdays ~ stategini + stategini + stategini:stategini + over64 + college + stincpc +
             south, data=states)
summary(m2)
...
coef.gini <- data.frame(fake_stategini = seq(min(states$stategini), max(states$stategini), 
                        length.out=100), coef_gini = NA, ub_gini = NA, lb_gini = NA)


for(i in 1:100) {   
    coef.gini$coef_gini[i] <- mean(m2.sims@coef[,2] + coef.gini$fake_stategini[i]*m2.sims@coef[,8])
    coef.gini$ub_gini[i] <- quantile(m2.sims@coef[,2] + coef.gini$fake_stategini[i]*m2.sims@coef[,8], .975)
    coef.gini$lb_gini[i] <- quantile(m2.sims@coef[,2] + coef.gini$fake_stategini[i]*m2.sims@coef[,8], .025)    
}

gini.coef.plot <- ggplot(coef.gini, aes(x = fake_stategini, y = coef_gini)) + 
